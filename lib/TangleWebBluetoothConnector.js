...
class WebBLEConnection extends TangleConnectorFlags {
  ...
  constructor() {
    ...
    this.TRANSFER_TYPE_DELIVER = 1; // deliver makes sure message was received (TCP style)
    this.TRANSFER_TYPE_TRANSMIT = 2; // transmit is fire and forget (UDP style)
    this.TRANSFER_TYPE_REQUEST = 3; // request requests data from firmware (HTTP style)
    ...
    */
    ...
    /*
      queue of commands (deliver, transmit, request) to be proccessed
    */
  ...
  transmit(command_data, command_label) {
    if (!this.#writing) {
      return this.#process(new Command(this.TRANSFER_TYPE_TRANSMIT, command_data, command_label));
    } else {
      return Promise.reject("Communication in proccess");
    }
  }

  // request first writes the request to the Terminal Characteris
...
class TangleWebBluetoothConnector {
  ...
  constructor() {
    ...
    this.transmitter = new WebBLEConnection();
...
*/
  ...
  autoSelect(criteria, scan_period = 1000, timeout = 3000) {
    // let options = {
    //   filters: [
    //     { namePrefix: "NARA", services: ["cc540e31-80be-44af-b64a-5d2def886bf5"] },
    //     //{services: ["cc540e31-80be-44af-b64a-5d2def886bf5"]}
    //   ],
    //   //optionalServices: ["cc540e31-80be-44af-b64a-5d2def886bf5"]
    // };
    ...
    if (this.webBTDevice && JSON.stringify(criteria) === JSON.stringify(this.#criteria)) {
      console.log("Reconnecting to the new device");
  ...
  connect(attempts=3) {
    ...
    return this.webBTDevice.gatt
      ...
      .then((service) => {
        ...
        return this.transmitter.attach(service, this.TERMINAL_CHAR_UUID, this.CLOCK_CHAR_UUID, this.DEVICE_CHAR_UUID);
      })
      .then(() => {
  ...
  // disconnect Connector from the connected Tangle Device
  disconnect() {
  ...
  transmit(command_payload, command_label) {
    ...
    return this.transmitter.transmit(command_payload, command_label);
  }
