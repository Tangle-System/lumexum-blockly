...
class QueueItem {
  ...
  // static TYPE_TRANSMIT = 3; // transmit is fire and forget (UDP style)
  static TYPE_REQUEST = 2; // request requests data from firmware ("HTTP style")
  // static TYPE_CONFIGURE = 4; // configure configures the device and reads its response ("HTTP style")

  constructor(type, payload, label, read_response) {
    this.type = type;
...
class WebBLEConnection {
  ...
  attach(service, networkUUID, clockUUID, deviceUUID) {
    ...
    return this.#service
      ...
      .then(() => {
        ...
        this.#networkChar.addEventListener("characteristicvaluechanged", this.#onNetworkNotification);
      })
      .catch((e) => {
        console.warn(e);
      })
      ...
      .then(() => {
        ...
        this.#networkChar.addEventListener("characteristicvaluechanged", this.#onDeviceNotification);
      })
      .catch((e) => {
        console.warn(e);
      })
    ...
    if (!this.#writing) {
      ...
      (async () => {
        while (this.#queue.length > 0) {
          ...
          switch (command.type) {
            case QueueItem.TYPE_DELIVER:
              await this.#writeBytes(this.#networkChar, command.payload /*, true*/)
                .then(() => {
                  command.resolve();
                })
                .catch((error) => {
            ...
            //   await this.#writeBytes(this.#networkChar, command.payload, false)
            //     .then(() => {
            //       command.resolve();
            //     })
            //     .catch((error) => {
              ...
              await this.#writeBytes(this.#deviceChar, command.payload /*, true*/)
                .then(() => {
                  if (command.read_response) {
                    return this.#readBytes(this.#deviceChar).then((response) => {
  ...
  deliver(payload, payload_label = null) {
    const command = new QueueItem(QueueItem.TYPE_DELIVER, payload, payload_label, false);

    // there must only by one deliver command in the queue with given label
  ...
  request(payload, read_response) {
    const item = new QueueItem(QueueItem.TYPE_REQUEST, payload, null, read_response);
...
*/
  ...
  deliver(payload, payload_label = null) {
    ...
    return this.#connection.deliver(payload, payload_label);
  }
