...
var bt_clock = new TimeTrack();

function getClockTimestamp() {
  return bt_clock.millis();
}

function TangleBluetoothDevice() {
  this.bluetoothConnection = new TangleBluetoothConnection();

  this.bluetoothConnection.addEventListener("#disconnected", (e) => {
    this.onDisconnect(e);
  });
  this.bluetoothConnection.addEventListener("#connected", (e) => {
    this.onConnect(e);
  });
...
TangleBluetoothDevice.prototype.onDisconnect = function (event) {
  ...
  this.bluetoothConnection.eventEmitter.emit("disconnected", { target: this });
  ...
  if (event.target.connected) {
    setTimeout(() => {
      ...
      return event.target
        .reconnect()
        .then(() => {
          return event.target.getClock();
        })
        .then((clock) => {
          bt_clock = clock;
          this.bluetoothConnection.eventEmitter.emit("connected", { target: this });
        })
        .catch((error) => {
          console.error(error);
          event.target.reset();
  ...
  return this.bluetoothConnection
    ...
    .then(() => {
      return this.bluetoothConnection
        .connect()
        .then(() => {
          return this.bluetoothConnection.getClock();
        })
        .then((clock) => {
          bt_clock = clock;
          this.bluetoothConnection.eventEmitter.emit("connected", { target: this });
