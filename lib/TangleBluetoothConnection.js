...
Transmitter.prototype._writeFirmware = function (firmware) {
  return new Promise(async (resolve, reject) => {
    const data_size = detectAndroid() ? 992 : 4992; // must be modulo 16
  ...
  if (!detectBluefy()) {
    this.BLE_OPTIONS = {
      ...
      filters: [
        ...
        { services: [this.FW_POST_0_7_SERVICE_UUID] },
...
TangleBluetoothConnection.prototype.scan = function () {
  ...
  return navigator.bluetooth.requestDevice(this.BLE_OPTIONS).then((device) => {
    ...
    this.bluetoothDevice.addEventListener("gattserverdisconnected", (e) => {
...
Transmitter.prototype._getUUID = function() {
  // valid UUIDs are in range [1..4294967295] (32 bit number)
  if (this._uuidCounter >= 4294967295) {
    this._uuidCounter = 0;
  }
...
Transmitter.prototype._writeBytes = function (characteristic, bytes, response) {
  const write_uuid = this._getUUID();
...
Transmitter.prototype.readClock = function () {
  //console.log("sync(" + timestamp +")");
...
TangleBluetoothConnection.prototype.getClock = function () {
