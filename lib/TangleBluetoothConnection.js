...
Transmitter.prototype._writeFirmware = function (firmware) {
  return new Promise(async (resolve, reject) => {
...

    {
      ...
      await this._writeBytes(this._updateChar, bytes, true);
    }
...
// sync() synchronizes the device clock
Transmitter.prototype.deviceReboot = async function () {
...
Transmitter.prototype._writeBytes = function (characteristic, bytes, response) {
  ...
  const packet_size = 512; // min size packet_header_size + 1
  //const packet_size = 192; // !!!! ANDROID NEEDS PACKET SIZE < 200 (192 seems to work well) !!!!
  const bytes_size = packet_size - packet_header_size;
    ...
    while (index_from < bytes.length) {
      ...
      try {
        if (response) {
          ...
          await characteristic.writeValueWithoutResponse(new Uint8Array(payload));
        }
      } catch (e) {
        error = e;
        break;
