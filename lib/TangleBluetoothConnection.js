...
Transmitter.prototype.attach = function (service) {
  ...
  return this._service.getCharacteristic(this.TERMINAL_CHAR_UUID)
    .then((characteristic) => {
      this._terminalChar = characteristic;
      return this._terminalChar.startNotifications();
    })
    .then(() => {
      console.log("> Terminal notifications started");
      this._terminalChar.addEventListener("characteristicvaluechanged", this._onTerminalNotification);
    })
    .catch((e) => {
      console.error(e);
    })
    ...
    .then((characteristic) => {
      ...
      return this._syncChar.startNotifications();
    })
    .then(() => {
      console.log("> Sync notifications started");
      this._syncChar.addEventListener("characteristicvaluechanged", this._onSyncNotification);
    })
    .catch((e) => {
      console.error(e);
    })
    ...
    .then((characteristic) => {
      ...
      return this._updateChar.startNotifications();
    })
    .then(() => {
      console.log("> Update notifications started");
      this._updateChar.addEventListener("characteristicvaluechanged", this._onUpdateNotification);
    })
    .catch((e) => {
      console.error(e);
    })
...
Transmitter.prototype._onTerminalNotification = function (event) {
...
Transmitter.prototype._onSyncNotification = function (event) {
  let value = event.target.value;
  let a = [];
  for (let i = 0; i < value.byteLength; i++) {
    a.push("0x" + ("00" + value.getUint8(i).toString(16)).slice(-2));
...
Transmitter.prototype._onUpdateNotification = function (event) {
  let value = event.target.value;
  let a = [];
  for (let i = 0; i < value.byteLength; i++) {
    a.push("0x" + ("00" + value.getUint8(i).toString(16)).slice(-2));
