...
// TangleDevice.js -> TangleAccessInterface.js -> TangleXXXConnector.js ->
...
// funkce jsou tim padem nejspi async a musi se volat s await.
// userSelect, autoSelect, selected
// destroy, 
...
class TangleDevice {
  ...
  constructor() {
    ...
    this.interface = new TangleAccessInterface();
  ...
  adopt(newDeviceName, newDeviceId) {
    ...
    return this.connector
      ...
      .then(() => {
        ...
        return this.connector.request(bytes, true).then((response) => {
  ...
  updateNetworkFirmware(firmware) {
    return new Promise(async (resolve, reject) => {
      const chunk_size = detectAndroid() ? 976 : 4976; // must be modulo 16
...

      {
        ...
        await this.connector.deliver(network_bytes, null);
      }
      ...
      await this.connector.deliver(payload, false);
  ...
  updateConfig(config) {
    ...
    return this.connector
      ...
      .then(() => {
        ...
        return this.connector.request(payload, false);
      });
  }
  ...
  //       const bytes = [NETWORK_FLAGS.FLAG_ADJUST_CLOCK, ...numberToBytes(device_clock_delta, 4)];
  ...
  syncClock() {
    ...
    return this.connector.setClock(this.clock).then(() => {
      console.log("> Device clock synchronized");
    });
  }
  ...
  reboot() {
    ...
    const payload = [NETWORK_FLAGS.DEVICE_CONF_BYTES, ...numberToBytes(1, 4), DEVICE_FLAGS.FLAG_DEVICE_REBOOT];
    return this.connector.deliver(payload, false);
  }
