...
// TangleSerialDevice.prototype.connected = function () {
...
//   const timeline_bytes = [TNGL_FLAGS.FLAG_SET_TIMELINE, ...numberToBytes(getClockTimestamp(), 4), ...numberToBytes(timeline_timestamp, 4), flags];
...
//   const payload = [TNGL_FLAGS.FLAG_SET_TIMELINE, ...numberToBytes(getClockTimestamp(), 4), ...numberToBytes(timeline_timestamp, 4), flags];
//   this.serialConnection.transmitter.deliver(payload);
...
//   const payload = [TNGL_FLAGS.FLAG_EMIT_TIMESTAMP_EVENT, ...numberToBytes(event_value_timestamp, 4), ...labelToBytes(event_label), ...numberToBytes(event_timestamp, 4), device_id];
//   this.serialConnection.transmitter.deliver(payload);
...
//   const payload = [TNGL_FLAGS.FLAG_EMIT_COLOR_EVENT, ...colorToBytes(event_value), ...labelToBytes(event_label), ...numberToBytes(event_timestamp, 4), device_id];
//   this.serialConnection.transmitter.deliver(payload);
...
//   const payload = [TNGL_FLAGS.FLAG_EMIT_PERCENTAGE_EVENT, ...percentageToBytes(event_value), ...labelToBytes(event_label), ...numberToBytes(event_timestamp, 4), device_id];
//   this.serialConnection.transmitter.deliver(payload);
...
//   const payload = [TNGL_FLAGS.FLAG_EMIT_LABEL_EVENT, ...labelToBytes(event_value), ...labelToBytes(event_label), ...numberToBytes(event_timestamp, 4), device_id];
//   this.serialConnection.transmitter.deliver(payload);
...
// //     const e = events[i];
// //     const bytes = [TNGL_FLAGS.FLAG_EMIT_EVENT, e.device_id, e.code, e.parameter, ...numberToBytes(e.timeline_timestamp, 4)];
// //     payload.push(...bytes);
// //   }
...
//   this.serialConnection.transmitter.writeClock(getClockTimestamp());
//   return true;
// };
