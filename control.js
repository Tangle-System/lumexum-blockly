...
window.onload = function () {
  ...
  let musicEventsRunning = false;

  function handleControlSoundMusic(){

      if(musicEventsRunning){
        musicEventsRunning = false;
        control_sound_music.textContent ="Music OFF";
      } else{
        musicEventsRunning = true;
        control_sound_music.textContent ="Music ON";
      }
      ...
      var webaudio_tooling_obj = function () {
        ...
        start_fft(myAudioElement.captureStream());
        ...
        // TODO refactor this function to be usable for other audio sources
        function start_fft(stream){
  ...
  control_sound_music.onclick = e => handleControlSoundMusic();
...
function handleFileDrop(e) {
  ...
  if(e.dataTransfer.files[0].name.endsWith(".tgbl")) {
    parseInputXMLfile(e.dataTransfer.files[0]);
  } else if (e.dataTransfer.files[0].name.endsWith(".zip")) {
    parseInputXMLfile(e.dataTransfer.files[0]);
  } else {
    const music_url = URL.createObjectURL(e.dataTransfer.files[0]);
    // Code.music.setAttribute("src", music_url);
    wavesurfer.load(music_url);
  }
}
...
document.querySelectorAll("body *").forEach(e => e.addEventListener('drop',handleFileDrop)); 
...
document.querySelector('.tabmax').addEventListener('click',preventFocus);